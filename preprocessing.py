...
NORMALIZED_DATA_DIR = "data/normalized_all"
...
def _find_users_with_most_photos(circled_images_paths: List[str],
                                 num_users_wanted: int = 10) -> List[str]:
    """
...
def split_system_users(circled_images_dir: str = CIRCLED_DATA_DIR,
    ...
    top_users_ids = _find_users_with_most_photos(circled_images_paths, number_of_users_wanted)

    top_users_image_paths = [path for path in circled_images_paths if extract_user_sample_ids(path)[0] in top_users_ids]
    ...
    registered_dir = os.path.join(system_database_dir, "registered_users")
    unknown_dir = os.path.join(system_database_dir, "unknown_users")

    create_empty_dir(normalized_images_dir)
    create_empty_dir(registered_dir)
    create_empty_dir(unknown_dir)

    for image_path in tqdm(top_users_image_paths, desc="Copying irides images"):
        ...
        # Corrected to use the circled images directory for the source
        original_image_path = os.path.join(circled_images_dir, file_name)

        # Ensure the directory for the target_dir exists
        if not os.path.exists(target_dir):
            os.makedirs(target_dir)
        # Construct the destination file path
        destination_path = os.path.join(target_dir, file_name)

        # Save original image to the system database directory
        copyfile(original_image_path, destination_path)
        ...
        if is_registered:
            ...
            normalized.save(os.path.join(normalized_images_dir, file_name))
