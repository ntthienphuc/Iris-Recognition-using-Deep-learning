...
class Image:
    ...
    def draw_circle(self, circle: Circle, color=(0, 0, 255), thickness=2):
        # Ensure the center coordinates are integers
        center = (int(circle.center_x), int(circle.center_y))
        # Explicitly cast radius to integer to avoid OpenCV error
        radius = int(circle.radius)
        
        # Draw the circle using OpenCV, with center coordinates and explicitly casted radius
        updated_img = cv2.circle(self.img, center, radius, color, thickness=thickness)
        
        # Return a new Image instance with the updated image
        return Image(updated_img)
